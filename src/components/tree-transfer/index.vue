    ...
    <template v-if="mode == 'transfer'">
      ...
      <div class="transfer-left">
        <h3 class="transfer-title">
           <el-radio-group v-model="yhzlx" size="small" @change='changeXzlb'>
             <el-radio v-for="item in fromTitleList" :key="item.value" :label="item.value">{{ item.name }}</el-radio>
              <!-- <el-radio label="user" >{{ fromTitle[0] }}</el-radio>
              <el-radio label="dept" >{{ fromTitle[1] }}</el-radio> -->
              <!-- <el-radio label="auto" >{{ fromTitle[2] }}</el-radio> -->
            </el-radio-group>
...
export default {
  ...
  computed: {
    fromTitleList: function() {
      ...
      let fromTitleList = []
      list.forEach(item => {
        if(this.xzlbList.indexOf(item.value) > -1) {
          fromTitleList.push(item)
        }
      })
      return fromTitleList
    },
    // 左侧数据
    self_from_data() {
