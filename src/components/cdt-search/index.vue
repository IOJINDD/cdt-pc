        ...
        >
          ...
          >
            ...
            >
              ...
              ></el-tree>
              <div
                class="el-input el-input--mini el-popover__reference"
                slot="reference"
                @click="handleShow(item)"
              >
                <div class="el-input__inner dapartTree text-overflow">
                  <!-- {{ searchForm[item.key] || (treeObj[item.key] && treeObj[item.key].label) || item.placeholder }} -->
                  {{ searchForm[item.key] ? glbmDwdm[searchForm[item.key]] : (treeObj[item.key] && glbmDwdm[treeObj[item.key].value]) || item.placeholder }}
                </div>
              </div>
            </el-popover>
          </el-form-item>
...
export default {
      ...
      handler(val) {
        if (!this.isSearch) {
          ...
          this.timeout = setTimeout(() => {
            if (
              this.searchForm.hasOwnProperty("time") &&
              this.searchForm["time"]
            ) {
              // 加两个new Date() 是因为把时间放在路径上传过去会变成string类型，加一个new Date() 把string类型转成Date 类型
              this.searchForm.kssj = this.searchForm["time"][0]
                ? new Date(
                    new Date(this.searchForm["time"][0]).getTime()
                  ).format("yyyy-MM-dd")
                : null;
              this.searchForm.jssj = this.searchForm["time"][1]
                ? new Date(
                    new Date(this.searchForm["time"][1]).getTime()
                  ).format("yyyy-MM-dd")
                : null;
            } else {
              if (this.searchForm.hasOwnProperty("kssj")) {
  ...
  methods: {
    ...
    glbmListTreeCurrentChangeHandle(data, node) {
      ...
      console.log("treeObj", this.treeObj);
    ...
    // 刷新
    handleRefresh() {
      if (this.searchForm.hasOwnProperty("time") && this.searchForm["time"]) {
        this.searchForm.kssj = this.searchForm["time"][0]
          ? new Date(this.searchForm["time"][0].getTime()).format("yyyy-MM-dd")
          : null;
        this.searchForm.jssj = this.searchForm["time"][1]
          ? new Date(this.searchForm["time"][1].getTime()).format("yyyy-MM-dd")
          : null;
      } else {
        if (this.searchForm.hasOwnProperty("kssj")) {
          this.searchForm.kssj = null;
    ...
    // 回车事件
    keyupEnter() {
      if (this.searchForm.hasOwnProperty("time")) {
        this.searchForm.kssj = this.searchForm["time"][0]
          ? new Date(this.searchForm["time"][0].getTime()).format("yyyy-MM-dd")
          : null;
        this.searchForm.jssj = this.searchForm["time"][1]
